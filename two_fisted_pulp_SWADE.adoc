...
=== Obstacles and Complicaions in Chases

As per the rules, if the action card that is dealt to a character at the start of a chase round (SWADE p 114) is a club, there's a complication.

In standard chases, you check the chase card suite against the Complications table to determine the mechanical effect of the complication and the hero makes a maneuvering roll (SWADE p 116).

In many of the set-piece chases that _Daring Tales of Adventure_ scenarios are known for, the GM refers to the Obstacles listed for the scene. When the action card is a club, look up the card value to determine what happens next.footnote:[See "Daring Tales of Adventure - The Devil's Chalice",  Act 1, Scene 2] The obstacle descriptions provide nice narrative color, indicate the maneuvering penalty, and state what happens if the maneuvering roll is a failure.

In some _Daring Tales of Adventure_ chase scenes, the obstacles and events of a chase are pre-ordained and the heroes are running a gauntlet.
